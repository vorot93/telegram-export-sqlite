...
fn parse_regular_message(
    ...
    let sender_name = string_field(source_json, "from").map(ToOwned::to_owned);
    ...
    parsed.messages.push(Message {
        ...
        telegram_message_id: telegram_message_id.unwrap_or(ordinal),
        sender_name,
...
fn parse_service_message(
    ...
    let event_type = string_field(source_json, "action")
        ...
        .to_string();
    let actor_name = string_field(source_json, "actor").map(ToOwned::to_owned);
    let target_names = string_array_field(source_json, "members");
...
fn parse_attachments(
    ...
    if source_json.get("contact_information").is_some()
        || source_json.get("contact_vcard").is_some()
    {
...
mod tests {
    ...
    fn parses_representative_json_export() {
        ...
        assert_eq!(first_message.telegram_message_id, 101);
