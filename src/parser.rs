...
fn parse_message_element(
    ...
    parsed.messages.push(Message {
        ...
        reactions: parse_reactions(body),
        extra_json: json!({
            "classes": class_attr(element),
            "source_anchor": source_anchor(element),
            "sender": sender_extra,
...
fn selector(selector_text: &str) -> Selector {
    Selector::parse(selector_text).expect("parser selector compiles")
}
...
mod tests {
    ...
    fn does_not_infer_sender_for_non_joined_message_without_from_name() {
        ...
        let second = parsed
            .messages
            .iter()
            .find(|message| message.telegram_message_id == 2)
            .unwrap();
    ...
    fn preserves_raw_markup_for_reconstructable_message_and_service_details() {
        ...
        let message = parsed
            .messages
            .iter()
            .find(|message| message.telegram_message_id == 401)
            .unwrap();
        assert!(
