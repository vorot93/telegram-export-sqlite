use crate::{
    export_rows::{AttachmentRow, ExportRows, MessageRow, PollOptionRow, PollRow, ServiceEventRow},
    media_path::safe_href,
...
pub fn render_llm(rows: &ExportRows, transcripts: &HashMap<String, String>) -> String {
    ...
    let referenced: HashSet<i64> = rows
        ...
        .collect();
    // `anchors` maps a telegram_message_id to its `#n` for reply resolution and
    // points at the FIRST row bearing that id. `anchor_at_item` records which
    // timeline row actually owns the anchor, so a conflict-kept duplicate that
    // shares the same id never also prints `#n`.
    let mut anchors: HashMap<i64, usize> = HashMap::new();
    let mut anchor_at_item: HashMap<i64, usize> = HashMap::new();
    ...
    for item in &rows.timeline_items {
        ...
        // Only a message that actually has a Telegram id can be a reply target.
        if let Some(id) = message.telegram_message_id
            && referenced.contains(&id)
            && !anchors.contains_key(&id)
        {
            anchors.insert(id, next_anchor);
            anchor_at_item.insert(item.id, next_anchor);
            next_anchor += 1;
        }
    }
    ...
    for item in &rows.timeline_items {
        ...
        if item.item_kind == "message" {
            ...
            if let Some(n) = anchor_at_item.get(&item.id) {
...
mod tests {
    use super::*;
    use crate::export_rows::{AttachmentRow, PollOptionRow, PollRow};
    ...
    fn code_and_pre_entities_escape_embedded_backticks() {
        ...
        assert_eq!(
            render_message_text(r#"[{"type":"code","text":"plain"}]"#, None),
            "`plain`"
        );
    }
    ...
    #[test]
    fn conflict_kept_duplicate_ids_do_not_both_claim_the_anchor() {
        let mut rows = empty_rows();
        // Two messages kept as a conflict share telegram_message_id 500; a later
        // message replies to 500. The `#1` anchor must mark exactly one of them,
        // not both — otherwise `↳#1` points at two different messages.
        rows.timeline_items = vec![
            timeline(1, 1, "message", Some("2026-05-30T09:15:00Z")),
            timeline(2, 2, "message", Some("2026-05-30T09:16:00Z")),
            timeline(3, 3, "message", Some("2026-05-30T09:20:00Z")),
        ];
        let first = message(1, 500, "Alice", "question");
        let dup = message(2, 500, "Alice", "same id different row");
        let mut reply = message(3, 501, "Bob", "answer");
        reply.reply_to_message_id = Some(500);
        rows.messages = vec![first, dup, reply];
        ...
        assert_eq!(
            out.matches(" #1").count(),
            1,
            "the #1 anchor must mark exactly one message:\n{out}"
        );
        assert!(
            out.contains("↳#1"),
            "the reply must still resolve to #1:\n{out}"
        );
    }
    ...
    fn service_text_newlines_are_indented_not_column_zero() {
        let mut rows = empty_rows();
        rows.timeline_items = vec![timeline(
            1,
            1,
            "service_event",
