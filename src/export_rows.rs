...
pub struct TimelineRow {
    ...
    /// Not populated by the export loader: neither exporter reads it, and it
    /// embeds a copy of the source HTML, so loading it wasted memory. Always
    /// empty here (the persisted `timeline_items.extra_json` column is untouched).
...
pub struct MessageRow {
    ...
    /// Not populated by the export loader (see `TimelineRow::extra_json`).
    pub extra_json: String,
}
...
fn load_timeline_rows(conn: &Connection) -> Result<Vec<TimelineRow>> {
    let mut stmt = conn.prepare(
        "SELECT ti.id, ti.ordinal, ti.item_kind, ti.source_anchor,
                ...
                u.display_name, ti.display_text
...
mod tests {
    ...
    fn loads_export_rows_in_timeline_order() {
        ...
        assert_eq!(export.messages[0].telegram_message_id, Some(101));
