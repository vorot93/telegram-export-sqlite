    ...
    match sources {
        ExportSources::Html(source_files) => {
            ...
            for source_file in source_files {
                ...
                db::insert_source_file(&tx, import_id, source_file, chat_title)?;
                db::insert_parsed_export(
                    &tx,
                    import_id,
        ...
        ExportSources::Json(source_files) => {
            ...
            let chat_title = parsed.chat.as_ref().map(|chat| chat.title.as_str());
            db::insert_source_file(&tx, import_id, source_file, chat_title)?;
            db::insert_parsed_export(&tx, import_id, source_files, &parsed)?;
            summary.files_imported += 1;
            add_parsed_counts(&mut summary, &parsed);
        }
    }
...
fn temporary_database_path(output_db: &Path) -> PathBuf {
    ...
    let file_name = output_db
        ...
        .unwrap_or_else(|| "export.sqlite".into());
