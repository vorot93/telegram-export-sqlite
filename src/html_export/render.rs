use crate::{
    ...
    media_path::{encode_media_path, href_scheme, safe_href, safe_media_path},
    model::{TextEntity, TextEntityKind},
    reactions::{reaction_count, reaction_emoji},
    time::parse_utc,
};
...
fn render_message_html(
    ...
    // A message with no Telegram id carries no numeric anchor: emit no `id` so a
    // round-trip re-imports it as id-less (None) rather than re-fabricating one
    // from the ordinal (parse_message_id reads this attribute back). Id-less
    // messages are never reply targets.
    let id_attr = match message.telegram_message_id {
        Some(id) => format!(" id=\"message{id}\""),
        None => String::new(),
    };
    let mut html = format!("\n    <div class=\"{class_name}\"{id_attr}>\n");
...
/// The href/src for an attachment's media, relative to the exported
/// `messages.html`. The canonical source is `relative_path` (validated, and copied
/// into the output tree by `html_export`); it is percent-encoded so filenames with
/// `#`/`?`/`%`/space resolve (C46). The original export href preserved in
/// `extra_json` is deliberately ignored: it is relative to the source chat's own
/// messages file and points nowhere from our single root-level `messages.html`.
fn attachment_render_path(attachment: &AttachmentRow) -> Option<String> {
    let safe = safe_media_path(attachment.relative_path.as_deref()?)?;
    Some(encode_media_path(&safe))
}
...
fn render_text_entity(entity: &TextEntity) -> String {
    ...
    match &entity.kind {
        ...
        TextEntityKind::Url => {
            // Telegram auto-links a bare domain ("example.com") as https. Without
            // a scheme the browser resolves the href relative to the file://
            // export, so the link is dead; add https:// when the URL text carries
            // no scheme of its own (an explicit http/https/… is left untouched).
            if href_scheme(&entity.text).is_some() {
                render_safe_link(&entity.text, &text)
            } else {
                render_safe_link(&format!("https://{}", entity.text), &text)
            }
        }
...
mod tests {
    ...
    #[test]
    fn bare_domain_url_entities_get_an_https_scheme() {
        let entities = vec![
            // A bare domain: Telegram auto-links it as https. Without a scheme the
            // browser resolves the href relative to the file:// export → dead link.
            TextEntity {
                kind: TextEntityKind::Url,
            ...
            // An explicit scheme is left exactly as-is.
            TextEntity {
                kind: TextEntityKind::Url,
                text: "http://foo.test/x".to_string(),
                extra: json!({}),
            },
        ];
        ...
        assert_eq!(
            html,
            "<a href=\"https://example.com\">example.com</a> and <a href=\"http://foo.test/x\">http://foo.test/x</a>"
        );
    }
...
mod timeline_tests {
    ...
    #[test]
    fn id_less_message_renders_without_a_numeric_anchor() {
        // A message with no Telegram id must not emit id="messageN": parse_message_id
        // reads that attribute back, so a numeric anchor (or an ordinal stand-in)
        // would re-fabricate the id on round-trip. Id-less messages are never
        // reply targets, so they need no anchor at all.
        let mut id_less = message_row(0, 1, "Alice", "no id");
        id_less.telegram_message_id = None;
        let rows = ExportRows {
            ...
            timeline_items: vec![TimelineRow {
                ...
                original_timestamp: None,
                actor_name: Some("Alice".to_string()),
                display_text: Some("no id".to_string()),
                extra_json: "{}".to_string(),
            }],
            messages: vec![id_less],
            service_events: vec![],
            attachments: vec![],
            polls: vec![],
            poll_options: vec![],
        ...
        // The message div is rendered, but with the `>` right after the class —
        // i.e. no `id="message…"` attribute.
        assert!(
            rendered
                .html
                .contains("class=\"message default clearfix\">"),
            "id-less message must render with no numeric anchor:\n{}",
            rendered.html
        );
    }
    ...
    fn renders_reply_forward_media_poll_service_and_reactions() {
        ...
        // The reply href keeps tdesktop's `#go_to_message{id}` form: the parser
        // (`a[href*="go_to_message"]`) reads it back on re-import, so the exporter
        // must emit it too. The onclick drives the JS scroll to `message{id}`.
    ...
    #[test]
    fn extra_json_href_is_ignored_and_relative_path_governs_the_link() {
        // The original export href in extra_json is never rendered (relative_path is
        // canonical), so even a parent-traversal href cannot leak into output; the
        // safe relative_path is what is linked.
        let rows = ExportRows {
            chat_title: "Family Chat".to_string(),
    ...
    #[test]
    fn extra_json_href_percent_encoded_traversal_forms_are_ignored() {
        // Same guard for percent-encoded traversal forms in the ignored extra_json
        // href: they never appear because the href is not consulted at all.
        let rows = ExportRows {
            chat_title: "Family Chat".to_string(),
    ...
    #[test]
    fn pathless_attachment_renders_no_link_even_with_extra_json_href() {
        // With no relative_path there is nothing safe to link, and the extra_json
        // href (here URL/absolute/protocol-relative forms) is ignored, so the
        // attachment renders as a plain box with no href.
        let rows = ExportRows {
            chat_title: "Family Chat".to_string(),
    ...
    #[test]
    fn attachment_href_uses_relative_path_and_ignores_stale_extra_json_href() {
        // The canonical media reference is `relative_path` (validated, and copied
        // into the export output next to messages.html), not the original export
        // href preserved in extra_json. That href is relative to the source chat's
        // own messages file (e.g. "files/report.pdf" under chat_001/) and points
        // nowhere from our single root-level messages.html, so it must be ignored
        // even when present and syntactically safe.
        let rows = ExportRows {
            chat_title: "Family Chat".to_string(),
            timeline_items: vec![timeline_message(20, 120, "Ivan", "Doc")],
            messages: vec![message_row(120, 20, "Ivan", "Doc")],
            service_events: vec![],
            attachments: vec![file_attachment(
                20,
                Some("chat_001/files/report.pdf"),
                "files/report.pdf",
            )],
            polls: vec![],
            poll_options: vec![],
        ...
        assert!(rendered.html.contains("href=\"chat_001/files/report.pdf\""));
        // The stale original href (relative to the source messages file) is not used.
        assert!(!rendered.html.contains("href=\"files/report.pdf\""));
    }

    #[test]
    fn attachment_href_percent_encodes_unsafe_characters() {
        // C46: a real filename containing '#'/space/'%'/'?' must be percent-encoded,
        // or the browser truncates the href at the fragment/query and the link dies.
        let rows = ExportRows {
            chat_title: "Family Chat".to_string(),
            timeline_items: vec![timeline_message(21, 121, "Judy", "Doc")],
            messages: vec![message_row(121, 21, "Judy", "Doc")],
            service_events: vec![],
            attachments: vec![file_attachment(21, Some("files/a b#c.pdf"), "")],
            polls: vec![],
            poll_options: vec![],
        };
        ...
        assert!(rendered.html.contains("href=\"files/a%20b%23c.pdf\""));
        assert!(!rendered.html.contains("a b#c.pdf"));
    }
