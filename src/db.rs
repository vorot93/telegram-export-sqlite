...
pub fn create_schema(conn: &Connection) -> Result<()> {
    ...
    conn.execute_batch(
        ...
        CREATE TABLE IF NOT EXISTS source_files (
            ...
            -- No UNIQUE(import_id, relative_path): a merge copies an input's source files
            -- (which for a previously merged input span several original imports) under one
            -- import, so the same relative_path legitimately recurs. The importer dedups
            -- source files explicitly and never relies on this constraint.
        ...
        CREATE UNIQUE INDEX IF NOT EXISTS users_by_telegram_id
            ON users(telegram_user_id) WHERE telegram_user_id IS NOT NULL;
        CREATE UNIQUE INDEX IF NOT EXISTS users_by_display_name
            ON users(display_name) WHERE telegram_user_id IS NULL;
        ...
        CREATE TABLE IF NOT EXISTS messages (
            ...
            -- Nullable: a message without a Telegram id is stored as NULL rather
            -- than a fabricated stand-in (the nullable timeline_items copy is the
            -- single authoritative id; see AGENTS.md).
            telegram_message_id INTEGER,
...
fn insert_timeline_item(
    ...
    let actor_user_id =
        optional_user_id(conn, item.actor_name.as_deref(), item.actor_id.as_deref())?;
...
fn insert_message(
    ...
    let sender_user_id = optional_user_id(
        conn,
...
fn insert_service_event(
    ...
    let actor_user_id = optional_user_id(conn, service_event.actor_name.as_deref(), None)?;
...
pub(crate) fn optional_user_id(
    ...
    let telegram_user_id = telegram_user_id.filter(|id| !id.is_empty());
    ensure_user(conn, display_name, telegram_user_id).map(Some)
}
...
fn ensure_user(
    ...
    // Identity is the Telegram id when present (so two people sharing a display name,
    // and distinct deleted accounts, stay separate); otherwise the display name (HTML
    // exports carry no id). The partial unique indexes back each lookup.
    match telegram_user_id {
        Some(telegram_user_id) => {
            conn.execute(
                "INSERT OR IGNORE INTO users (telegram_user_id, display_name) VALUES (?1, ?2)",
        ...
        None => {
            ...
            Ok(conn.query_row(
                "SELECT id FROM users WHERE telegram_user_id IS NULL AND display_name = ?1",
                [display_name],
                |row| row.get(0),
            )?)
        }
...
mod tests {
    ...
    #[test]
    fn users_are_deduplicated_by_telegram_id_not_display_name() {
        let conn = Connection::open_in_memory().unwrap();
        create_schema(&conn).unwrap();

        // Two different people who both display as "Alex" are distinct users.
        let alex1 = ensure_user(&conn, "Alex", Some("user111")).unwrap();
        let alex2 = ensure_user(&conn, "Alex", Some("user222")).unwrap();
        assert_ne!(alex1, alex2);
        // The same Telegram id resolves to the same user despite a display-name change.
        assert_eq!(
            ensure_user(&conn, "Alexander", Some("user111")).unwrap(),
            alex1
        );
        // Without an id (HTML exports) identity falls back to the display name.
        let bob = ensure_user(&conn, "Bob", None).unwrap();
        assert_eq!(ensure_user(&conn, "Bob", None).unwrap(), bob);
        assert_ne!(bob, alex1);
    }
