...
Telegram HTML message files in natural split-file order. Each database holds a
single chat: a full-account JSON export (one containing many chats under
`chats.list`) is refused — re-export the individual chat you want to archive.
...
- `--force`: replace an existing output database (in-place mode) or `DEST`
  ...
  preserved.
- `--incremental`: refresh an existing database, skipping source files that were
  imported unchanged before. The existing database must be the current schema
  version and the same chat as the export — a different chat is refused rather
  than silently overwritten. A bundle refresh keeps previously archived media
  that a later export no longer includes, and recovers a bundle stranded by an
...
The exporter writes `messages.html`, its support assets, and a copy of every
referenced media file at its stored relative path so the links resolve:
...
js/script.js
images/...
chat_001/photos/photo_1.jpg   # referenced media, copied preserving its path
files/report.pdf
```

Referenced media is read from beside `<INPUT_DB>` (a bundle keeps it under
`assets/`; an in-place import keeps the original export layout), so keep the
database next to its media. Media the database no longer has beside it is skipped
and its link is left best-effort; the summary reports how many files were copied.
...
- HTML export (`export-html`) copies every referenced media file into the output
  directory at its stored `relative_path`, and percent-encodes the paths in the
  generated links, so `messages.html` resolves against local files. Media is read
  from beside the input database and the original export href in `extra_json` is
  ignored (it is relative to the source chat's own message files). A file no
  longer present beside the database is skipped rather than copied.
...
HTML export (`export-html`) likewise copies referenced media into its output
directory, so treat an exported HTML directory as a sensitive archive too.
