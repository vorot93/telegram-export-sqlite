...
fn cli_export_html_exports_imported_sqlite_to_html() {
    ...
    // Media is linked by its canonical relative_path and copied into the output so
    // the link resolves (the chat_001/ prefix is the per-chat dir the importer records).
    assert!(html.contains("href=\"chat_001/photos/photo_1.jpg\""));
    assert!(output.join("chat_001/photos/photo_1.jpg").is_file());
...
fn cli_export_html_exports_json_imported_sqlite_to_html() {
    ...
    assert!(output.join("files/report.pdf").is_file());
...
fn exported_html_reimports_with_same_core_semantics() {
    ...
    // Round-trip now PRESERVES the media path: export-html links media by its
    // canonical relative_path (`chat_001/photos/photo_1.jpg`) and copies the file
    // there, so re-importing that HTML recovers the same path. Previously the stale
    // extra_json href degraded it to `photos/photo_1.jpg` and copied nothing.
    let reimported_media_path: String = reimported
        .query_row(
            "SELECT relative_path FROM attachments WHERE relative_path = 'chat_001/photos/photo_1.jpg'",
            [],
            |row| row.get(0),
        )
        .unwrap();
    assert_eq!(reimported_media_path, "chat_001/photos/photo_1.jpg");
}
